...
def makeCLI() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--ontology", 
        type=str, 
        default=f"{__dirname}/medical.ttl", 
        help="The original ontology to load and process."
    )
    p.add_argument("--dataset", 
        type=str, 
        default=f"{__dirname}/Disease_Syntoms.csv", 
...
def main():
    ...
    try:
        ...
        for (k,v) in diseases.items(): 
            ...
            diff = new - existing
            for s in diff: 
                g.add((PREFIX[s], r.RDF.type, PREFIX["Symptom"]))
                g.add((PREFIX[trueKey], PREFIX["hasSymptom"], PREFIX[s]))

            g.add((PREFIX[trueKey], r.RDF.type, PREFIX["Disease"]))

...
ORDER BY ?id
    """
    res = execQuery(q)
    fields = getFields(q, res, ["livro", "titulo", "nomeAutor", "paisOrigemAutor", "linhaTemporal", "eventoId"])
...
@appRouter.get("/linha/<linha>")
def linha(linha):
    q = f"""
PREFIX : <{RDF_PREFIX}>
SELECT ?linha ?livro ?tituloLivro ?tipoLivro WHERE {{
    BIND(:{linha} AS ?linha)
...
ORDER BY ?livro
    """
    res = execQuery(q)
    fields = getFields(q, res, ["linha", "livro", "tituloLivro"])
    if (len(fields) == 0): return abort(404)

    print(jsonprint(f"{fields}".replace("'", '"')))
    field = fields[0]
    return RENDER("app/linha", {
        "entryName": field["linha"].short.replace(":", ""),
        "entryValues": {
            "_main": {},
            **{ k:v for (k,v) in map(
                lambda f: (f"Livro: {f['tituloLivro']}", pickFromObj(f, "livro", "tituloLivro", "tipoLivro")), 
                fields
            ) }
        },
        "entryKeyMap": {

# pyright: reportUnknownParameterType=false, reportUnknownMemberType=false, reportUnknownArgumentType=false, reportMissingParameterType=false, reportPrivateUsage=false, reportUnannotatedClassAttribute=false, reportExplicitAny=false, reportAny=false
...
class Logger:
    ...
    def __init__(
        ...
        self._levelnameSize = max(map(lambda n: len(n), self.levelnames.values()))  # pyright: ignore[reportUnknownLambdaType]
    ...
    def debug(self, *args: Any): self._log(Logger.LOG_DEBUG, args)
    def info(self, *args: Any): self._log(Logger.LOG_INFO, args)
    def warn(self, *args: Any): self._log(Logger.LOG_WARN, args)
    def error(self, *args: Any): self._log(Logger.LOG_ERROR, args)
    def log(self, *args: Any): self._log(Logger._LOG_LEVEL_DEFAULT, args)
